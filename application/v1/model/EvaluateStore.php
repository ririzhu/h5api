...
class EvaluateStore extends Model
{
     ...
     */
    public function getEvaluateStoreInfoByStoreID($vid, $sc_id = 0) {
        $prefix = 'evaluate_store_info';
        $base =new Base();
        $info = $base->rcache($vid, $prefix);
        if(empty($info)) {
            $info = array();
            $info['store_credit'] = $this->_getEvaluateStore(array('seval_storeid' => $vid));
//            $info['store_credit_average'] = round((($info['store_credit']['store_desccredit']['credit'] + $info['store_credit']['store_servicecredit']['credit'] + $info['store_credit']['store_deliverycredit']['credit']) / 3), 1);
            $info['store_credit_average'] = round((($info['store_credit']['store_desccredit']['credit'] + $info['store_credit']['store_servicecredit']['credit'] + $info['store_credit']['store_deliverycredit']['credit']) / 2), 1);
            //$info['store_credit_average'] = round((($info['store_credit']['store_desccredit']['credit'] + $info['store_credit']['store_servicecredit']['credit'] + $info['store_credit']['store_deliverycredit']['credit']) / 2), 1);
...
//        $result['store_deliverycredit']['text'] = '发货速度';
        $result['store_servicecredit']['text'] = '环境评分';
        $result['store_deliverycredit']['text'] = '教室评分';
