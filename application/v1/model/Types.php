...
class Types extends Model
{
     ...
     * @return array 二位数组
     */
    public function getAttrs($type_id, $vid, $gc_id) {
        ...
        if (is_array($spec_list) && !empty($spec_list)) {
            ...
            foreach ($spec_list as $val) {
                $spec_value_list = Model('spec')->getSpecValueList(array('sp_id'=>$val['sp_id'], 'gc_id'=>$gc_id, 'vid' => $vid));
                $a = array();
     ...
     * @return boolean
     */
    public function addGoodsType($gid, $commonid, $param) {
        // 商品与属性对应
        $sa_array = array();
        $sa_array['gid']       = $gid;
        $sa_array['goods_commonid'] = $commonid;
        $sa_array['gc_id']          = $param['cid'];
     ...
     * @return Array
     */
    public function typeRelatedList($table, $param, $field = '*',$page=''){
        $condition_str = $this->getCondition($param);
        $array = array();
        $array['table']		= $table;
        $array['where']		= $condition_str;
    ...
    public function typeRelatedGroupList($param, $type='') {
        ...
        switch ($type){
            ...
            case 'attr':
                ...
                $array['field']		= 'gid, attr_value_id';
                break;
            case 'brand':
                $array['table']		= 'goods';
                $array['field'] 	= 'gid, brand_id';
                if (isset($param['limit'])){
                    $array['limit'] 	= $param['limit'];
                }
                break;
     ...
     * @return bool
     */
    public function delAttrValue($table,$param){
        $condition_str = $this->getCondition($param);
        return Db::delete($table, $condition_str);
    }
    /**
    ...
    private function getCondition($condition_array) {
        $condition_str = '';
        if($condition_array['gid'] != ''){
            $condition_str .= " and gid ='".$condition_array['gid']."'";
        }
        if($condition_array['in_goods_id'] != ''){
            $condition_str .= " and gid in (".$condition_array['in_goods_id'].")";
        }
