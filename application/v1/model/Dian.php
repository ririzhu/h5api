...
class Dian extends Model
{
    ...
    public function getOwnShopIds($bind_all_gc = false) {
        ...
        if (!$data) {
            ...
            $data = $this->H('own_shop_ids')? $this->H('own_shop_ids'):$this->H('own_shop_ids',true);
            // 缓存为空则查库
            if (!$data) {
                $data = array();
                $all_own_shops = $this->table('bbc_vendor')->field('vid,bind_all_gc')->where(array(
                    'is_own_shop' => 1,
                ))->select();
    ...
    public function getDianCountByGid($gid,$regions=null,$condition=array()) {
        ...
        $result = $this->name('dian,dian_goods')->join('left')->on('dian.id=dian_goods.dian_id')->field("dian.id")->group('dian.id')->where($condition)->count();
