...
class MyGoods extends Model
{
    public function __construct(){
        parent::__construct('bbc_vendor_innercategory');
    }
    /**
     * 单个类别内容提取
     *
    ...
    public function getClassInfo($param,$field='*') {
        ...
        $param	= array();
        $param['table']	= 'bbc_vendor_innercategory';
    ...
    public function addGoodsClass($param) {
        ...
        $class_array['vid']	= $_SESSION['vid'];
        $class_array['stc_sort']	= $param['stc_sort'];

        $result	= Db::insert('bbc_vendor_innercategory',$class_array);
        if($result) {
            return true;
        } else {
            return false;
    ...
    public function editGoodsClass($param,$class_id) {
        ...
        $state	= Db::update('bbc_vendor_innercategory',$class_array," WHERE stc_id='$class_id'");
        if($state) {
            return true;
        } else {
            return false;
    ...
    public function dropGoodsClass($store_class_id) {
        ...
        foreach ($array as $val) {
            $class_info	= $this->getClassInfo(array('stc_id'=>$val,'vid'=>$_SESSION['vid']),'stc_id');
            if($class_info['stc_id'] != '') {
                Db::delete('bbc_vendor_innercategory'," WHERE stc_id='$val'");
            }
        }
        return true;
    }
    ...
    public function getClassList($condition){
        ...
        $result	= $this->table("bbc_vendor_innercategory")->where($where)->order($order)->select();
        return $result;
    }
    /**
     * 取分类列表(前台店铺页左侧用到)
     ...
     */
    public function getShowTreeList($vid){
        $show_class = array();
        $class_list = $this->getClassList(array('vid'=>$vid,'stc_state'=>'1','order'=>'stc_parent_id asc,stc_sort asc,stc_id asc'));
    ...
    private function getCondition($condition_array){
        ...
        if(!empty($condition_array)) {
            if (isset($condition_array['stc_top']) && $condition_array['stc_top'] != '') {
                $condition_sql .= " and stc_parent_id=0";
            }
            if (isset($condition_array['stc_id']) && $condition_array['stc_id'] != '') {
                $condition_sql .= " and stc_id= '{$condition_array['stc_id']}'";
            }
            if (isset($condition_array['vid']) && $condition_array['vid'] != '') {
                $condition_sql .= " and vid= '{$condition_array['vid']}'";
            }
            if (isset($condition_array['stc_parent_id']) && $condition_array['stc_parent_id'] != '') {
                $condition_sql .= " and stc_parent_id= '{$condition_array['stc_parent_id']}'";
            }
            if (isset($condition_array['stc_state']) && $condition_array['stc_state'] == '0' || $condition_array['stc_state'] == '1') {
