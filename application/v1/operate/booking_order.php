...
defined('DYMall') or exit('No Permission!');
class order_book {
    ...
    public function buy($order_data = array(), $post_data = array()) {
        ...
        if ($post_data['book_pay_type'] == 'part') {
            ...
            $data['book_store_id'] = $order_info['vid'];
            ...
            if (empty($book_list[0]['book_pay_time'])) {
                ...
                if ($book_list[0]['book_end_time'] > TIMESTAMP) {
                    ...
                    if ($order_info['order_state'] == ORDER_STATE_CANCEL) {
                        ...
                        $book_list[0]['book_operate'] = '<a class="sldm-btn sldm-btn-orange fr mr15" href="index.php?app=buy&mod=pay&pay_sn='.$pay_sn.'">付款</a>';
                    ...
                    $book_list[0]['book_amount_ext'] = '(含定金'.sldPriceFormat($book_list[0]['book_deposit_amount']).')';
                } else {
                    //已过期
                ...
                if (empty($book_list[1]['book_pay_time'])) {
                    ...
                    if ($book_list[1]['book_end_time'] > TIMESTAMP) {
                        ...
                        if ($order_info['order_state'] == ORDER_STATE_CANCEL) {
                            ...
                            $book_list[1]['book_operate'] = '<a class="sldm-btn sldm-btn-orange fr mr15" href="index.php?app=buy&mod=pay&pay_sn='.$pay_sn.'">付款</a>';
    ...
    public function changeBookOrderReceivePay($order_info,$post) {
        ...
        if ($book_list[0]['book_step']) {
            ...
            if (empty($book_list[0]['book_pay_time'])) {
                ...
                Model('goods')->editGoods(array('book_buyers'=>array('exp','book_buyers+1')),array('gid'=>$order_goods_info['gid']));
            } else {
    ...
    public function changeOrderStateCancel($order_info, $role, $user = '', $msg = '') {
        try {
            ...
            $result = Logic('queue')->cancelOrderUpdateStorage(array('id'=>$order_info['dian_id'],'data'=>$data));
            if (!$result['state']) {
