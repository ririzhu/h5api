    ...
    public function handle_buy_list($store_cart_list,$member_id){
        ...
        foreach ($store_cart_list as $k => $v) {
            ...
            if($_first[$k]) {
                ...
                foreach ($v as $kk => $vv) {
                    //如果该产品属于首单免费产品
                    if (isset($_first[$k][$vv['goods_commonid']])) {
                        $vvv=$_first[$k][$vv['goods_commonid']];
                        //给店铺免单赋值
                        if($this_reduct == null){
...
//                                dd('产品剩 0');
    ...
    public function have_qualified($member_id,$cid,$vid){
        ...
        $where['vid'] = $vid;
        $have = DB::table('bbc_order')->where($where)->count();
        ...
        $have = DB::table('bbc_first_discount')->where($where)->select();
