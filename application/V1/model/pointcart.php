...
class pointcart extends Model
{
	...
	public function getPointCartList($condition,$page='',$field='*'){
		...
		$param['field'] = $field;
		$param['order'] = $condition['order'] ? $condition['order'] : 'points_cart.pgid desc';
	...
	private function getCondition($condition_array){
		...
		if ($condition_array['pcart_id']) {
			$condition_sql	.= " and pcart_id = '{$condition_array['pcart_id']}'";
		}
		//会员编号
		...
		//积分礼品记录编号
		if ($condition_array['pgid']) {
			$condition_sql	.= " and pgid = '{$condition_array['pgid']}'";
		}
