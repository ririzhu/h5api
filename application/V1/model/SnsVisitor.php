...
class SnsVisitor{
	 ...
	 */
	public function getVisitorList($condition,$limit=10,$page='',$field='*'){
		/*$condition_str	= $this->getCondition($condition);
		$param	= array();
		$param['table']	= 'sns_visitor';
		$param['where']	= $condition_str;
		$param['field'] = $field;
		$param['order'] = isset($condition['order']) ? $condition['order'] : 'sns_visitor.v_addtime desc';
		$param['limit'] = isset($condition['limit']) ? $condition['limit'] : '';
		$param['group'] = isset($condition['group']) ? $condition['group'] : '';
		return Db::select($param,$page);*/
        return DB::name('sns_visitor')->field($field)->where($condition)->page($page)->limit($limit)->select();
	}
	/**
	 * 获取访客记录详细
	 * 
