...
class buy_virtual {
     ...
     * @return array
     */
    public function getBuyStep1Data($gid, $quantity, $member_id) {
        return $this->getBuyStepData($gid, $quantity, $member_id);
    }
     ...
     * @return array
     */
    public function getBuyStep2Data($gid, $quantity, $member_id) {
        return $this->getBuyStepData($gid, $quantity, $member_id);
    }
     ...
     * @return array
     */
    public function getBuyStepData($gid, $quantity, $member_id) {
        $goods_info = Model('goods')->getVirtualGoodsOnlineInfoByID($gid);
        if (empty($goods_info)) {
        ...
        //取得抢购信息
        $goods_info = $this->_getTuanInfo($goods_info);
        ...
        $goods_info['goods_total'] = sldPriceFormat($goods_info['goods_price'] * $goods_info['quantity']);
        $goods_info['goods_image_url'] = cthumb($goods_info['goods_image'], 240, $goods_info['vid']);

        $return = array();
        $return['goods_info'] = $goods_info;
        $return['store_info'] = Model('vendor')->getStoreOnlineInfoByID($goods_info['vid'],'store_name,vid,member_id');
...
//        }
    ...
    public function buyStep3($post, $member_id) {
        ...
        $result = $this->getBuyStepData($post['gid'], $post['quantity'], $member_id);
        ...
        $vid = $goods_info['vid'];
        $store_goods_total_list = array($vid => $pay_total);
        $pay_total = $store_goods_total_list[$vid];
        ...
        //变更库存和销量
        QueueClient::push('createOrderUpdateStorage', array($goods_info['gid'] => $goods_info['quantity']));

        //更新抢购信息
        $this->_updateTuan($goods_info);
    ...
    private function _createOrder($input, $goods_info, $member_info) {
        ...
        if ($goods_info['iftuan'] == 1) {
            $order['order_promotion_type'] = 1;
            $order['promotions_id'] = $goods_info['tuan_id'];
        }
     ...
     * 更新抢购购买人数和数量
     */
    private function _updateTuan($goods_info) {
        if ($goods_info['iftuan'] && $goods_info['tuan_id']) {
            $tuan_info = array();
            $tuan_info['tuan_id'] = $goods_info['tuan_id'];
            $tuan_info['quantity'] = $goods_info['quantity'];
    ...
    private function _rcbPay($order_info, $input, $buyer_info) {
        ...
        if ($available_rcb_amount >= $order_amount) {
            ...
            if($order_info['dian_id']>0) { //门店修改待自提
                $order_info['order_state'] = $data_order['order_state'] = ORDER_STATE_SEND;
            }else{
    ...
    private function _pdPay($order_info, $input, $buyer_info) {
        ...
        if ($available_pd_amount >= $order_amount) {
            ...
            $data_order = array();
            if($order_info['dian_id']>0) { //门店修改待自提
                $data_order['order_state'] = ORDER_STATE_SEND;
            }else{
     ...
     * @return array
     */
    private function _getTuanInfo($goods_info = array()) {
        if (!C('tuan_allow') || empty($goods_info) || !is_array($goods_info)) return $goods_info;
    
        $tuan_info = Model('tuan')->getTuanInfoByGoodsCommonID($goods_info['goods_commonid']);
        if (empty($tuan_info)) return $goods_info;
        // 虚拟抢购数量限制
        if ($tuan_info['upper_limit'] > 0 && $tuan_info['upper_limit'] < $goods_info['virtual_limit']) {
            $goods_info['virtual_limit'] = $tuan_info['upper_limit'];
        }
        $goods_info['goods_price'] = $tuan_info['tuan_price'];
        $goods_info['tuan_id'] = $tuan_info['tuan_id'];
