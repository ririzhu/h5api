...
class buy {
     ...
     */
    public function buyStep1($cart_id, $ifcart, $member_id, $vid) {

        //得到购买商品信息
        if ($ifcart) {
            $result = $this->getCartList($cart_id, $member_id);
        } else {
            $result = $this->getGoodsList($cart_id, $member_id, $vid);
        }
        ...
        //得到页面所需要数据：收货地址、发票、优惠券、预存款、商品列表等信息
     ...
     * @param int $vid 店铺编号
     */
    public function getGoodsList($cart_id, $member_id, $vid) {
        ...
        //商品信息[得到最新商品属性及促销信息]
        $goods_info = $this->_logic_buy_1->getGoodsOnlineInfo($gid,intval($quantity));
        if(empty($goods_info)) {
    ...
    public function buyStep2($post, $member_id, $member_name, $member_email) {
        ...
        try {
            ...
            //第3步 得到购买相关金额计算等信息
            $this->_createOrderStep3($member_id);
    ...
    public function changeAddr($freight_hash, $city_id, $area_id, $member_id) {
        ...
        //$offline_store_id_array = Model('vendor')->getOwnShopIds();
    ...
    private function _createOrderStep1() {
        ...
        if (!empty($post['voucher']) && is_array($post['voucher'])) {
            foreach ($post['voucher'] as $vid => $voucher) {
                if (preg_match_all('/^(\d+)\|(\d+)\|([\d.]+)$/',$voucher,$matchs)) {
                    if (floatval($matchs[3][0]) > 0) {
                        $input_voucher_list[$vid]['voucher_t_id'] = $matchs[1][0];
                        $input_voucher_list[$vid]['quan_price'] = $matchs[3][0];
                    }
                }
            }
        }
        ...
        $this->_order_data['order_from'] = $post['order_from'] == 1 ? 1 : 0;
     ...
     *
     */
    private function _createOrderStep3($member_id) {
        ...
        //计算店铺分类佣金[改由任务计划]
//         $store_gc_id_commis_rate_list = Model('vendor_bind_category')->getStoreGcidCommisRateList($goods_list);
...
//         $this->_order_data['store_gc_id_commis_rate_list'] = $store_gc_id_commis_rate_list;
    ...
    private function _createOrderStep4() {
        ...
        foreach ($store_cart_list as $vid => $goods_list) {
            ...
            $order['shipping_fee'] = $store_freight_total[$vid];
            ...
            $order_common['order_message'] = $input_pay_message[$vid];
    
            //优惠券
            if (isset($input_voucher_list[$vid])){
                $order_common['quan_price'] = $input_voucher_list[$vid]['quan_price'];
                $order_common['voucher_code'] = $input_voucher_list[$vid]['voucher_code'];
            }
            ...
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) {
                    ...
                    if ($goods_info['goods_storage_alarm'] >= ($goods_info['goods_storage'] - $goods_info['goods_num'])) {
                        ...
                        $notice_list['goods_storage_alarm'][$goods_info['vid']] = $param;
                    }
                    ...
                    foreach ($goods_info['bl_goods_list'] as $bl_goods_info) {
                        ...
                        if ($bl_goods_info['goods_storage_alarm'] >= ($bl_goods_info['goods_storage'] - $goods_info['goods_num'])) {
                            ...
                            $param['sku_id'] = $bl_goods_info['gid'];
                            $notice_list['goods_storage_alarm'][$bl_goods_info['vid']] = $param;
                        }
                    }
                }
            }
            ...
            //存储商家发货提醒数据
            if ($store_pay_type_list[$vid] == 'offline') {
                $notice_list['new_order'][$order['vid']] = array('order_sn' => $order['order_sn']);
            }
        }
        ...
        $this->_order_data['iftuan'] = $iftuan;
    }
    ...
    private function _createOrderStep6() {
        ...
        $iftuan = $this->_order_data['iftuan'];
            ...
            foreach ($store_cart_list as $goods_list) {
                foreach ($goods_list as $goods_info) {
                    if ($goods_info['iftuan'] && $goods_info['tuan_id']) {
                        ...
                        $tuan_info['quantity'] = $goods_info['goods_num'];
        ...
        @setBbcCookie('cart_goods_num','',-3600);
        ...
        if (!empty($notice_list)) {
            foreach ($notice_list as $code => $value) {
                QueueClient::push('sendStoreMsg', array('code' => $code, 'vid' => key($value), 'param' => current($value)));
            }
        }
