...
class ssys_cwap_homeModel extends Model{


    public function __construct() {
        parent::__construct('ssys_cwap_home');
    }
    ...
    public function getWapHomeList($condition, $page='', $order='home_id desc', $field='*') {
        ...
        $list = $this->table('ssys_cwap_home')->field($field)->where($condition)->page($page)->order($order)->select();
        return $list;
    }
    /**
     * 读取商城内置的手机模板
    ...
    public function getWapTemplateList($condition, $page='', $order='template_id asc', $field='*') {
        $list = $this->table('ssys_cwap_template')->field($field)->where($condition)->page($page)->order($order)->select();
        return $list;
    }
    /**
     * 获取首页详细信息  根据home_id
    ...
    public function getHomeInfoByHomeID($home_id) {
        ...
        $home_info = $this->table('ssys_cwap_home')->where($condition)->find();
        return $home_info;
    }
    /**
     * 获取首页详细信息  
    ...
    public function getTempInfoByID($id) {
        ...
        $temp_info = $this->table('ssys_cwap_template')->where($condition)->find();
        return $temp_info;
    }
    /*
     * 新增首页
    ...
    public function addHome($param,$condition=array()) {
        ...
        $result = $this->table('ssys_cwap_home')->where($condition)->update(array('home_state'=>0));
        $result = $this->table('ssys_cwap_home')->insert($param);
        if($result) {
            return $result;
        } else {
            return false;
    ...
    public function editCwapHome($update, $home_id) {
        ...
        $condition = array();
        $condition['home_id'] = $home_id;
        $result = $this->table('ssys_cwap_home')->where($condition)->update($update);
        if ($update['home_state']) {
            ...
            $result = $this->table('ssys_cwap_home')->where($where)->update($upData);
        }
        if($result) {
            return $home_id;
        } else {
    ...
    public function setCwapHome($home_id) {
        ...
        $result = $this->table('ssys_cwap_home')->where($condition_new)->update(array('home_state'=>1));
        if($result) {
            return $home_id;
        } else {
            return false;
    ...
    public function delCwapHomeByID($home_id) {
        ...
        return $this->table('ssys_cwap_home')->where($condition)->delete();
    }
    ...
    public function delCwapHomeByIDs($condition) {
        ...
        $result = $this->table('ssys_cwap_home')->where($condition)->delete();
        return $result;
    }
    ...
    {
        $condition['shop_id'] = isset($extends['shop_id']) ? $extends['shop_id'] : 0;
        $result = $this->table('ssys_cwap_home')->where($condition)->field('home_id,home_desc,home_state')->find();
        return $result;
    }
