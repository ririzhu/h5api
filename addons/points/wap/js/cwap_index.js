...
return i;
    ...
    //获取首页搜索栏颜色和标题
    $.ajax({
        url: ApiUrl + "/index.php?app=index&mod=index_title&sld_addons=points"+extend_param_str,
        type: 'get',
        dataType: 'json',
    ...
    // function load_tmp_data(extend_param_str,_sld_stats,lp=0){
        ...
        var load_html_obj;
        $.ajax({
            url: ApiUrl + "/index.php?app=index&sld_addons=points"+data_lp_str+extend_param_str,
            type: 'get',
            dataType: 'json',
            async: false,
            beforeSend: function(){
            ...
            success: function(result) {
                if(result.status == 155)
                {
                    ...
                    setTimeout(function(){window.location.href = '/cwap/index.html';},1500);
                    return;
                }
                // 关闭 加载中提示
                $(".sld-loading").remove();
