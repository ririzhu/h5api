...
class points_cwap_homeCtl extends SystemCtl{
	...
	public function getTemplatePages()
	{
		...
		$urlType = trim($_GET['urlType']) ? DS.$_GET['urlType'] : '/SystemManage';
		...
		foreach ($page_list as $key => $val){
			$page_list[$key]['decorate_url'] = MALL_URL.$urlType.'/index.php?app=cwap_home&mod=home_edit&id='.$val['home_id'].'&type=home&sld_addons=points';
		}
