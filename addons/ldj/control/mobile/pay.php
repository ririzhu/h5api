...
class payCtl extends mobileMemberCtl {
    ...
    public function pay_new() {
        ...
        if (in_array($_GET['payment_code'],array('alipay','weixin','yinlian','wxpay_jsapi','predeposit'))) {
            ...
            $mb_payment_info = $model_mb_payment->table('ldj_payment')->where($condition)->find();
...
//        $param['order_amount'] = $pay_info['data']['api_pay_amount']*100;
    ...
    public function pay_new_xcx() {
        ...
        if (in_array($_POST['payment_code'],array('mini_wxpay','predeposit'))) {
           $model_mb_payment = M('ldj_payment','ldj');
           $condition = array();
           $condition['payment_code'] = $_POST['payment_code'];
            $condition['payment_state'] = 1;
           $mb_payment_info = $model_mb_payment->getMbPaymentOpenInfo($condition);
           if(!$mb_payment_info) {
               echo json_encode(array('state'=>255,'msg'=>'支付方式未开启'));die;
           }

           $this->payment_code = $_POST['payment_code'];
           $this->payment_config = $mb_payment_info['payment_config'];
        }else {
            echo json_encode(array('state'=>255,'msg'=>'支付方式提交错误'));die;
        }
        if ($_POST['payment_code'] == 'predeposit') {
            try {
                $pay_info = $this->_get_real_order_info($pay_sn);
                if(isset($pay_info['error'])) {
                    echo json_encode(array('state'=>255,'msg'=>$pay_info['error']));die;
                }
                ...
                $data_order['pd_amount'] = $pay_info['api_pay_amount'];
                ...
                $order_model->commit();
                echo json_encode(['state'=>200,'msg'=>'支付成功']);die;
            } catch (Exception $e) {
                $order_model->rollback();
                echo json_encode(['state'=>255,'msg'=>'支付失败']);die;
            }
        }else{
            $pay_info = $this->_get_real_order_info($pay_sn);
            if(isset($pay_info['error'])) {
        ...
        switch ($mb_payment_info['payment_code']) {
            case 'alipay':
                ...
                if ($run_flag) {
                    ...
                    $pay_data['subject'] = $order_pay_info['pay_sn'];
            ...
            case 'wxpay_jsapi':
                ...
                if ($run_flag) {
                    ...
                    $all_wx_payment_config = M("ldj_payment",'ldj')->getWxpayAllConfig();
...
//        exit;
    ...
    private function getRealOrderInfo($pay_sn, $member_id)
    {
        ...
        $order_pay_info = $model_order->table('ldj_order_pay')->where($condition)->find();

        if(!empty($order_pay_info)){

            $order_pay_info['subject'] = '实物订单_'.$order_pay_info['pay_sn'];
            $order_pay_info['order_type'] = 'ldj_product_buy';

            $condition = array();
            $condition['pay_sn'] = $pay_sn;
            $order_list = $model_order->table('ldj_order')->where($condition)->select();
            //计算本次需要在线支付的订单总金额
            $pay_amount = 0;
            if (!empty($order_list)) {
