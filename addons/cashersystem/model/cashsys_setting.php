...
class cashsys_settingModel extends Model{
	public function __construct(){
		parent::__construct('cashsys_setting');
	}
	/**
	 * 读取系统设置信息
	 *
	...
	public function updateSetting($param){
		...
		if (is_array($param)){
			foreach ($param as $k => $v){
				...
				$where = " name = '". $k ."'";
				$result = Db::update('cashsys_setting',$tmp,$where);
				if ($result !== true){
					return $result;
				}
			}
			H('cashsys_setting',true);
			delete_file(BASE_DATA_PATH.DS.'cache'.DS.'cashsys_setting.php');
			return true;
		}else {
			return false;
		}
