 ...
 * Time: 11:23
 */
class pre_cwap_homeModel extends Model{


    public function __construct() {
        parent::__construct('cwap_home_pre');
    }
    ...
    public function getWapHomeList($condition, $page='', $order='home_id desc', $field='*') {
        ...
        $list = $this->table('cwap_home_pre')->field($field)->where($condition)->page($page)->order($order)->select();
        return $list;
    }
    /**
     * 读取商城内置的手机模板
    ...
    public function getWapTemplateList($condition, $page='', $order='template_id asc', $field='*') {
        $list = $this->table('cwap_template')->field($field)->where($condition)->page($page)->order($order)->select();
        return $list;
    }
    /**
     * 获取首页详细信息  根据home_id
    ...
    public function getHomeInfoByHomeID($home_id) {
        ...
        $home_info = $this->table('cwap_home_pre')->where($condition)->find();
        return $home_info;
    }
    /**
     * 获取首页详细信息
    ...
    public function addHome($param,$condition=array()) {
        ...
        $result = $this->table('cwap_home_pre')->where($condition)->update(array('home_state'=>0));
        $result = $this->table('cwap_home_pre')->insert($param);
        if($result) {
            return $result;
        } else {
            return false;
    ...
    public function editCwapHome($update, $home_id) {
        ...
        $condition = array();
        $condition['home_id'] = $home_id;
        $result = $this->table('cwap_home_pre')->where($condition)->update($update);
        if ($update['home_state']) {
            ...
            $result = $this->table('cwap_home_pre')->where($where)->update($upData);
        }
        if($result) {
            return $home_id;
        } else {
    ...
    public function setCwapHome($home_id) {
        ...
        $result = $this->table('cwap_home_pre')->where($condition_new)->update(array('home_state'=>1));
        if($result) {
            return $home_id;
        } else {
            return false;
    ...
    public function delCwapHomeByID($home_id) {
        ...
        return $this->table('cwap_home_pre')->where($condition)->delete();
    }
    ...
    public function delCwapHomeByIDs($condition) {
        ...
        $result = $this->table('cwap_home_pre')->where($condition)->delete();
        return $result;
    }
    ...
    public function getTemplateInfo($condition)
    {
        ...
        $result = $this->table('cwap_home_pre')->where($condition)->field('home_id,home_desc,home_state')->find();
        return $result;
    }
