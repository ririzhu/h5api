...
//        exit;
    ...
    private function getRealOrderInfo($pay_sn, $member_id)
    {
        ...
        $order_pay_info = $model_order->table('pre_order')->where($condition)->find();
        if(!empty($order_pay_info)){
            $order_pay_info['subject'] = '实物订单_'.$order_pay_info['order_sn'];
            $order_pay_info['order_type'] = 'pre_product_buy';
            //计算本次需要在线支付的订单总金额
            //判断是交定金还是交尾款
            if($order_pay_info['order_state'] == 10){
                ...
                $pay_amount = sldPriceFormat($buy_model->getPrePrice($order_pay_info['pre_id'],$order_pay_info['gid']) * $order_pay_info['goods_num']);
