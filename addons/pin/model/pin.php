...
//        }
	...
	public function getTuanList($condition, $page = null, $order = 'id asc', $field = '*', $limit = 0) {
        $tuan_list = $this->table('pin')->field($field)->where($condition)->page($page)->order($order)->limit($limit)->select();
        return $tuan_list;
	}
...
//        $where['goods.gid'] = array('neq','');
    ...
    public function getTuanInfo($condition) {
        $tuan_info = $this->table('pin')->where($condition)->find();
        return $tuan_info;
    }
    ...
    private function _getTuanListByGoodsid_new($gid) {
        ...
        $xianshi_goods_list = $this->table('pin_goods,pin')->join('right')->on('pin_goods.sld_pin_id=pin.id')
        ->where($condition)->order('pin.id')->select();
...
//        $condition['sld_start_time'] = array('lt', TIMESTAMP);
        $condition['pin.sld_end_time'] = array('gt', TIMESTAMP);
    ...
    public function addTuan($param){
        // 发布团购锁定商品
        return $this->table('pin')->insert($param);
    }
    ...
    public function editTuan($update, $condition) {
        return $this->table('pin')->where($condition)->update($update);
    }
