...
$(".loginlei").click(function(){
    ...
    if(code == "" || password == "" || phone == ""){
        ...
        var member_avatar=GetQueryString('member_avatar');
        var member_otherinfo=GetQueryString('member_otherinfo');

        var weixin_unionid=GetQueryString('weixin_unionid');

        $.ajax({
            type: "post",
            url: ApiUrl + '/index.php?app=login&mod=gzh_register&sld_addons=spreader',
            data:{'mobile':phone,'password':password,'client':'wap','vcode':code,'inviteid':decodeURIComponent(getcookie('inviteid')),'openid':openid,'member_avatar':member_avatar,'member_otherinfo':member_otherinfo,'weixin_unionid':weixin_unionid},
            dataType: "JSON",
            success: function(data){
