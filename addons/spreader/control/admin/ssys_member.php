...
class ssys_memberCtl extends SystemCtl{
    ...
    public function member(){
        ...
        if (is_array($member_list)){
            foreach ($member_list as $k=> $v){
                ...
                $item_condition['shop_member_id'] = $v['shop_member_id'];
				...
				//推手状态返回
				switch($v['ts_member_state']){
					case 0:$member_list[$k]['ssys_member_state'] = '申请推手';break;
					case 1:$member_list[$k]['ssys_member_state'] = '正式推手';break;
					default:
						$member_list[$k]['ssys_member_state'] = '永久推手';
				}
            }
        }else{
            $member_list = array();
    ...
    public function get_more_info_about_yj()
    {
    	...
    	if (in_array($detail_state, $allow_detail_states) && $member_id > 0) {
	    	...
	    	$yj_log_list = $ssys_yj->getPdLogList($condition,$pageSize,'*','lg_add_time desc');
