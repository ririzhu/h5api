...
class cwap_homeModel extends Model{
    ...
    public function __construct() {
        parent::__construct('cwap_home_pin');
    }
    ...
    public function getWapHomeList($condition, $page='', $order='home_id desc', $field='*') {
        ...
        $list = $this->table('cwap_home_pin')->field($field)->where($condition)->page($page)->order($order)->select();
        return $list;
    }
    /**
     * 读取商城内置的手机模板
    ...
    public function getHomeInfoByHomeID($home_id) {
        ...
        $home_info = $this->table('cwap_home_pin')->where($condition)->find();
        return $home_info;
    }
    /**
     * 获取首页详细信息
    ...
    public function addHome($param,$condition=array()) {
        ...
        $result = $this->table('cwap_home_pin')->where($condition)->update(array('home_state'=>0));
        $result = $this->table('cwap_home_pin')->insert($param);
        if($result) {
            return $result;
        } else {
            return false;
    ...
    public function editCwapHome($update, $home_id) {
        ...
        $condition = array();
        $condition['home_id'] = $home_id;
        $result = $this->table('cwap_home_pin')->where($condition)->update($update);
        if ($update['home_state']) {
            ...
            $result = $this->table('cwap_home_pin')->where($where)->update($upData);
        }
        if($result) {
            return $home_id;
        } else {
    ...
    public function setCwapHome($home_id) {
        ...
        $result = $this->table('cwap_home_pin')->where($condition_new)->update(array('home_state'=>1));
        if($result) {
            return $home_id;
        } else {
            return false;
    ...
    public function delCwapHomeByID($home_id) {
        ...
        return $this->table('cwap_home_pin')->where($condition)->delete();
    }
    ...
    public function delCwapHomeByIDs($condition) {
        ...
        $result = $this->table('cwap_home_pin')->where($condition)->delete();
        return $result;
    }
    ...
    public function getTemplateInfo($condition)
    {
        ...
        $result = $this->table('cwap_home_pin')->where($condition)->field('home_id,home_desc,home_state')->find();
        return $result;
    }
