...
<div class="tabmenu">
  <?php include template('layout/submenu');?>
    ...
    <a href="<?php echo urlAddons('add');?>" class="bbc_ms-btn bbc_ms-btn-green" title="新增团购"><i class="iconfontfa fa-plus-circle add_iconcolor"></i>新增阶梯团</a>
...
<table class="search-form">
  <form method="get">
      <input type="hidden" name="app" value="action_ladder" />
      <input type="hidden" name="mod" value="pin_list" />
      <input type="hidden" name="sld_addons" value="pin_ladder" />
    <tr>
    ...
    <td class="w100"><select name="type" class="w90">
            ...
            <?php if(is_array($output['types'])) { ?>
                <?php foreach($output['types'] as $key=>$val) { ?>
                    <option value="<?php echo $val['id'];?>" <?php if($val['id'] == $_GET['type']) { echo 'selected';}?>><?php echo $val['class_name'];?></option>
                <?php } ?>
            <?php } ?>
        </select></td>
      <th>商品名称</th>
...
<table class="bbc_ms-table-style">
  ...
  <tbody>
    ...
    <tr class="bd-line">
      ...
      <td><?php echo $group['class_name'];?></td>
