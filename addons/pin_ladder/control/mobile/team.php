...
class teamCtl extends mobileHomeCtl{
    ...
    public function detail() {
        ...
        for ($x=0; $x<$pin_info['sld_team_count']; $x++) {
            ...
            if($list[$x]){
                $list[$x]['avatar'] = $list[$x]['member_avatar']?UPLOAD_SITE_URL.DS.ATTACH_AVATAR.DS.$list[$x]['member_avatar']:getCimg('default_user_portrait');
            }else{
                if($pin_info['sld_tuan_status']!=1) {
