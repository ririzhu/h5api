...
class redModel {
    ...
    public function getRedUserList($condition, $page = null, $order = 'reduser_use desc,bbc_red_user.id asc') {
        ...
        $member_list = DB::table('bbc_member')->field('member_id,member_name,member_truename,wx_nickname')->where($where)->select();
        //优惠券信息数据
        $redinfo_ids = low_array_column($red_user_list,'redinfo_id');
        $where2['id'] = array('in',join(',',$redinfo_ids));
        $redinfo_list = DB::table('bbc_red_info')->where($where2)->select();
        $all_list = array();
        //全数据
